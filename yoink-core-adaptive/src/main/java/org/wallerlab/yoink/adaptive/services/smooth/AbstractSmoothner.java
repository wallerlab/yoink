...
import static org.wallerlab.yoink.api.model.region.Region.Name.QM_CORE;
/**
 ...
 *
 */
public abstract class AbstractSmoothner implements Smoothner {
	...
	/**
	 * AbstractSmoothner constructor with specified smooth function.
	 * 
	 * @param smoothFunction -the specified smooth function for a new object of AbstractSmoothner.
	 */
	public AbstractSmoothner(SmoothFunction smoothFunction) {
	...
	protected List<Double> loopOverAllBufferMolecules(Job job) {
		...
		Set<Molecule> qmCoreMolecules = regions.get(QM_CORE).getMolecules();
		Set<Molecule> bufferMolecules = regions.get(BUFFER).getMolecules();
		...
		for (Molecule molecule : bufferMolecules) {
			...
			getProperSmoothParameters(qmCoreMolecules, smoothParameters, molecule);
			double min = smoothParameters.get(0);
			double max = smoothParameters.get(1);
			double currentValue = smoothParameters.get(2);
			double smoothFactor = smoothFunction.evaluate(currentValue, min, max);
	...
	protected abstract void getProperSmoothParameters(
			Set<Molecule> qmCoreMolecules, List<Double> smoothParameters, Molecule molecule);
