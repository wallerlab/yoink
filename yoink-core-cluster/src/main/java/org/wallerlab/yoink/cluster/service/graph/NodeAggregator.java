...
public class NodeAggregator {
	...
	NodeAggregator(GraphDatabaseService graph, RelationshipType edgeType) {
		this.graph = graph;
	...
	List<Node> aggregateCommunities(Collection<Node> startNodes, long hierarchyLevel) {
		...
		for (Relationship edge : traversal.traverse(startNodes).relationships()) {
			...
			Double weight = Double.valueOf((double) edge.getProperty("weights", 1.0));
			...
			for (int i = 0; i <= 1; i++) {
				...
				if (!nodeSet.contains(node.getId())) {
					...
					node.createRelationshipTo(parent, LouvainRelation.LOUVAIN_ACCUMULATED_BY);
				}
			}
		}
