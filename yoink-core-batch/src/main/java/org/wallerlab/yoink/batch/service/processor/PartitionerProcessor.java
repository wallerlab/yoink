...
import static org.wallerlab.yoink.api.model.batch.JobParameter.PARTITIONER;
import static org.wallerlab.yoink.api.service.region.Partitioner.Type.NUMBER;
import static org.wallerlab.yoink.api.service.region.Partitioner.Type.SIZE;
 ...
 */
@Service
public class PartitionerProcessor implements ItemProcessor<Job<JAXBElement>,Job>{
	...
	@Autowired
	protected List<Regionizers> regionizers;


	//The idea is to have the driver here. This way the paritioners stay agnostic to how they are called.
	...
	public Job process(Job<JAXBElement> job) throws Exception {
		...
		for (Regionizer regionizer : regionizers) {
			regions = regionizer.regionize(regions, job.getParameters());
		}
