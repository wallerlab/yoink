...
import static org.wallerlab.yoink.api.model.region.Region.Name.QM_CORE_ADAPTIVE;
...
public class SeddPartitioner extends InteractionPartitioner {
	...
	protected Region checkCriteria(Map<Region.Name, Region> regions,
		...
		checkTwoAtomsDensity(regions, region, neighbours, parameters, neighbourAtoms, coordinate, densityOfTwoAtoms);
		return region;
	}
	...
	private double getTwoAtomsDensity(Set<Atom> neighbourAtoms, Coord coordinate) {
		...
		for (Atom atom : neighbourAtoms) {
			densityOfTwoAtoms += atomDensityCalculator.calculate(coordinate, atom);
		}
	...
	private void checkTwoAtomsDensityRatio(Map<Region.Name, Region> regions,
		...
		if (densityRatioOfTwoAtoms >= (double) parameters.get(DENSITY_RATIO_MIN)
