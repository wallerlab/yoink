...
public class GridPointAssigner implements
	 ...
	 */
	public Map assign(Coord gridPoint, Map<Region.Name, Region> regions, Region.Name cubeRegionName) {
		...
		Map<String, Object> properties = voronoiCalculator.calculate(gridPoint,molecules);
		//Map<String, Object> properties = voronoiFunctionalCalculator.calculate(gridPoint, molecules);
		...
		default:
			...
			boolean notValid = notNeighbourPair || bothNeighboursAreInNonQmCore || bothNeighboursAreInQmCore;
			if (notValid) properties.clear();
		}
		return properties;
	}
